...
When('I rename the facility to {string}', async ({ page }, newName: string) => {
	...
	await expect
		.poll(
			...
			{ timeout: 8_000, message: 'the rename neither redirected nor reported success' }
		)
		.not.toBe('pending');
});
...
When('I close the edit dialog', async ({ page }) => {
	...
	await expect(dialog(page)).toHaveCount(0, { timeout: 8_000 });
...
Then('no two facility buttons share the same link', async ({ page, baseURL }) => {
	...
	//
	// The prefix is part of the href the app renders -- links are built from
	// `base` -- so an attribute selector written for the bare path matches
	// nothing on a prefixed site and the step fails as "no facility buttons
	// found" rather than as a missing prefix.
	const sites = `${basePathOf(baseURL)}/sites`;
