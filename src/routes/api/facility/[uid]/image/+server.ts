...
export const PUT: RequestHandler = async ({ request, params, cookies }) => {
    ...
    if (typeof alt === 'string') {
        proxyFormData.append('alt', alt);
    }

    const response = await fetch(`${variables.BASE_URI}/api/v2/facilities/${uid}/image`, {
        method: 'PUT',
        headers: {
...
/** Corrects the picture's description without re-uploading the file. */
export const PATCH: RequestHandler = async ({ request, params, cookies }) => {
    ...
    const response = await fetch(`${variables.BASE_URI}/api/v2/facilities/${uid}/image`, {
        ...
        body: JSON.stringify({ alt: body.alt ?? '' })
    });
    ...
    if (!response.ok) {
        ...
        error(response.status, errorData.detail || 'Description update failed');
    }
