...
<button
	...
	title="Créer"
	><span><Fa icon={faPlus} /></span><span>Créer une personne physique ou morale</span></button
>
		...
		<div class="rounded-lg p-6 variant-ghost-secondary space-y-6 items-center place-items-center">
			...
			<div class="p-4 space-y-2 justify-items-stretch grid grid-cols-1 gap-6">
				<form {...createEffector} class="">
					...
					{#if createEffector.result?.success}
						<span class="badge-icon variant-filled-success"><Fa icon={faCheck} /></span>
					{:else if createEffector.result && !createEffector.result.success}
						<span class="badge-icon variant-filled-error"><Fa icon={faExclamationCircle} /></span
						>{createEffector.result.text}
					{/if}
				</div>
						...
						<div class="w-auto justify-center">
							<button
								...
								onclick={() => {
									dialog?.close();
									createdEffector = createEffector.result?.data;
								}}>{createEffector.result?.success ? 'Fermer' : 'Annuler'}</button
							>
						</div>
					</div>
				</form>
